...
class FFTr4(private val size: Int, private val tNum: Int = 1) {
    ...
    */
    ...
    fun execute(r: DoubleArray): Pair<DoubleArray, DoubleArray> {
        if (r.size != size) throw IllegalArgumentException("input length?")
        var outr = DoubleArray(size)
        ...
        for (i in 0 until r.size) {
            outr[i] = re[stepNum % 2][i]
            outi[i] = im[stepNum % 2][i]
